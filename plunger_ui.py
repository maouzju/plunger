...
class ProxyDashboard:
    ...
    def _stop_existing_proxy_before_start(self, previous_started_at_ms: int | None) -> None:
        if previous_started_at_ms is None:
            return
        ...
        if self._wait_for_proxy_exit(previous_started_at_ms, timeout=3.0):
            return
    ...
    def _wait_for_proxy_exit(self, expected_started_at_ms: int | None, timeout: float) -> bool:
        ...
        return self._fetch_health() is None and (
    ...
    def _is_previous_instance_health(self, health: dict[str, Any] | None) -> bool:
        previous_started_at_ms = self.pending_start_previous_started_at_ms
        current_started_at_ms = self._extract_started_at_ms(health)
        if previous_started_at_ms is None or current_started_at_ms is None:
            return False
